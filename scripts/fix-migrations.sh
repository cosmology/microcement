...
# Fix Liquibase Migration Script
# This script clears the changelog lock and runs migrations properly

echo "🔧 Fixing Liquibase Migration"
echo "============================="
...
# Clear Liquibase changelog lock
print_status "Clearing Liquibase changelog lock..."
docker compose exec db psql -U postgres -d postgres -c "DELETE FROM databasechangeloglock WHERE id = 1;" 2>/dev/null || true

# Run Liquibase migration using run --rm
